# -*- coding: utf-8 -*-
"""棘轮关采纳形态落库：bt_f70e4260e8b2 形态 + atr_trail_floor=False（分钟），全区间 + OOS。"""
import json
import sys
import uuid
...
def main():
    ...
    cfg = base_ratchet()  # 用户形态+P4+asym_bias0+fwd_t on+棘轮关
    full = json.loads(json.dumps(cfg))
    full["name"] = "pulse_ratchet_adopted_full"
    save("🏷️棘轮关采纳形态-全区间(分钟)", full)
    oos = json.loads(json.dumps(cfg))
    oos["start_date"] = OOS_SPLIT
    oos["name"] = "pulse_ratchet_adopted_oos"
    save("🏷️棘轮关采纳形态-OOS段(分钟)", oos)
