# -*- coding: utf-8 -*-
"""池级趋势开关语境实验：bt_76889c798212 形态上 pool_gate on→off。

基座 = bt_76889c798212（score+P4+asym_bias0+fwd_t on+棘轮关+t_debt_max_days=1，
pool_gate=True/enter_th 0.15）。语境变体 GATEOFF：顶层 pool_gate=False。
基线复用 pulse_debt_rows.jsonl（同基座 D1_full/D1_oos）。

流程：--stage1 语境双段验证 → --stage2 P0 五项 OAT → --ab 正增量双段。
判定纪律：AB 采纳线 = OOS 超额不低于语境基座。
注意：D-OAT 曾发现 pool_gate 在动态语境 I=0（增量归零）——本实验在分钟语境复核。
"""
import argparse
import json
import sys
...
ROWS_JSONL = OUT_DIR / "pulse_gateoff_rows.jsonl"
DEBT_ROWS = OUT_DIR / "pulse_debt_rows.jsonl"


def base_gate_on() -> dict:
    cfg = base_trend()
    cfg["params"]["t_debt_max_days"] = 1  # bt_76889c798212 采纳档
    cfg["name"] = "gateoff_base"
    return cfg


def apply_gateoff(cfg: dict, tag: str) -> dict:
    out = json.loads(json.dumps(cfg))
    out["pool_gate"] = False  # 顶层键
    out["name"] = tag
    return out
...
def main():
    ...
    # 基线：复用 pulse_debt_rows.jsonl（同基座 D1 双段）
    base = {}
    if DEBT_ROWS.exists():
    ...
    print(f"基线(pool_gate=on)：全区间 score {bf['score']:.4f}/"
          f"超额 {_pct(bf.get('excess_return'))}｜OOS 超额 {_pct(boos_ex)}", flush=True)

    cfg0 = base_gate_on()
    gcfg = apply_gateoff(cfg0, "gateoff_ctx")

    if args.stage1:
        run_and_log(apply_gateoff(cfg0, "gateoff_full"), "GATEOFF_full",
                    "GATEOFF语境 全区间")
        run_and_log(apply_gateoff({**cfg0, "start_date": OOS_SPLIT}, "gateoff_oos"),
                    "GATEOFF_oos", "GATEOFF语境 OOS")
        _report_stage1(done, bf, boos_ex)
        print(f"总耗时 {time.time()-t0:,.0f}s", flush=True)
        return

    gf, go = done.get("GATEOFF_full"), done.get("GATEOFF_oos")
    if not (gf and go):
        raise RuntimeError("缺 stage1 结果，先跑 --stage1")
    ok = (gf["score"] > bf["score"]
          and (go.get("excess_return") or -9) >= boos_ex)
    print(f"语境判定：GATEOFF vs 基座 → 全区间 Δscore {gf['score']-bf['score']:+.4f}｜"
          f"OOS Δ超额 {(go.get('excess_return') or 0)-boos_ex:+.2%} "
          f"({'✅ 语境成立' if ok else '❌ 语境不成立'})", flush=True)
    if not ok:
        print("语境不成立，stage2/ab 终止", flush=True)
        return

    bs, goos_ex = gf["score"], go.get("excess_return") or 0
    if args.stage2:
        total = len(_p0_variants())
        rows = []
        for i, (combo, ov) in enumerate(_p0_variants(), 1):
            r = run_and_log(_apply_ov(gcfg, ov, f"gateoffP0_{i}"), combo,
                            f"[{i}/{total}] {combo}")
    ...
    for combo, ov in winners:
        full = run_and_log(_apply_ov(gcfg, ov, f"gateoffAB_{combo}_full"),
                           f"AB_{combo}_full", f"[AB] {combo} 全区间")
        oos = run_and_log(_apply_ov(apply_gateoff(
            {**cfg0, "start_date": OOS_SPLIT}, "x"), ov,
            f"gateoffAB_{combo}_oos"), f"AB_{combo}_oos", f"[AB] {combo} OOS")
        ab_rows.append({"combo": combo, "full": full, "oos": oos})
        print(f"  {combo} 全区间 Δ超额="
              f"{(full.get('excess_return') or 0)-(gf.get('excess_return') or 0):+.2%} | "
              f"OOS Δ超额={(oos.get('excess_return') or 0)-goos_ex:+.2%}", flush=True)
...
def _report_stage1(done: dict, bf: dict, boos_ex: float) -> None:
    lines = [
        ...
        "- 基座 = bt_76889c798212 形态（pool_gate=True/enter_th 0.15）｜"
        "GATEOFF = 顶层 pool_gate=False",
        f"- 基线(pool_gate=on)全区间：{_rs(bf)}",
    ...
    for seg, b in (("full", bf), ("oos", None)):
        ...
        lines.append(
            f"| GATEOFF | {seg} | {r['score']:.4f} | {_pct(r.get('total_return'))} "
            f"| {_pct(r.get('excess_return'))} | {_pct(r.get('max_drawdown'))} | {tail} |")
    lines += ["", "## 判定", "",
              "- 全区间 Δscore>0 且 OOS Δ超额≥0 → 语境成立，进 stage2；否则终止"]
    out = OUT_DIR / f"pulse_gateoff_stage1_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
...
def _report_stage2(done: dict, rows: list, bs: float) -> None:
    lines = [
        "# P0 五项 OAT（GATEOFF 语境：pool_gate=off）",
        "",
        f"- 生成时间：{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
        f"- 语境基座：{_rs(done['GATEOFF_full'])}",
    ...
    out = OUT_DIR / f"pulse_gateoff_stage2_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
    out.write_text("\n".join(lines), encoding="utf-8")
    print(f"报告 → {out}", flush=True)


def _report_ab(done: dict, ab_rows: list) -> None:
    gf = done["GATEOFF_full"]
    lines = [
        "# GATEOFF 语境 AB 对照",
        "",
        f"- 生成时间：{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}｜OOS = {OOS_SPLIT} 起",
        f"- GATEOFF 语境基座 全区间：{_rs(gf)}",
        f"- GATEOFF 语境基座 OOS：{_rs(done['GATEOFF_oos'])}",
        "",
        "| 组合 | 段 | score | 超额 | vs语境基座超额 | 采纳 |",
        "|---|---|---|---|---|---|",
    ]
    for r in ab_rows:
        for seg, res, bref in (("全区间", r["full"], gf),
                               ("OOS", r["oos"], done["GATEOFF_oos"])):
    ...
    out = OUT_DIR / f"pulse_gateoff_ab_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
    out.write_text("\n".join(lines), encoding="utf-8")
    print(f"报告 → {out}", flush=True)
