...
- ✅ **数据管理页 minute5 统计冷盘慢（P1-5，已落地）**：`store.parquet_stats_minute5` 对约 3000 个分钟线文件逐读 footer，冷文件系统缓存下实测 **13.5s**（热态 0.44s），导致数据管理页打开卡顿。改为：① 线程池（16）并发读元数据（冷盘 IO 显著提速）；② 进程内 60s TTL 缓存（`_stats_cache`，同进程重复打开秒回，实测缓存命中 0.0000s）。TTL 内数据更新后行数可能短暂滞后，可接受。**后续可选**：更新完成时主动失效缓存（需在 updater 写文件后清 key）或持久化统计指纹。

- ✅ **盘前日线增量收窄（P1-6，已落地）**：`live_premarket_task` 的日线增量更新原为全市场 5000+ 只全拉，改为 **候选域指数成分 ∪ 当前池 ∪ 持仓 ∪ 状态机票**（实盘候选域锁定 zz500 时约 500 只），增量耗时降一个数量级。**已知限制**：① 候选域若扩展（auto_index 加指数/清空=全市场）或启用 `min_rps`（全市场 RPS 分位门槛）时，本收窄会缺数据——届时需改回 `codes=None` 全市场；② 成分历史快照缺失时已兜底全市场。
