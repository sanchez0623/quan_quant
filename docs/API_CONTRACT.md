...
| `GET /api/live/summary`              | 概览（池子/gate/持仓/信号/回填/feishu\_configured/config）+ **持仓盈亏聚合 `equity{cash, market_value, equity, total_pnl, total_pnl_pct}`**：每次调用顺带 qt 实时价刷持仓现价（分钟级节流落库，行情失败退库内快照）；浮盈=市值−成本（未扣卖出费用），与盘后对账卡同口径 |
