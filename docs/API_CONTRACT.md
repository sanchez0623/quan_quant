...
| `GET /api/live/summary`              | 概览（池子/gate/持仓/信号/回填/feishu\_configured/config）+ **持仓盈亏聚合 `equity{cash, market_value, equity, total_pnl, total_pnl_pct, day_pnl, dd_pct}`**：每次调用顺带 qt 实时价刷持仓现价（分钟级节流落库，行情失败退库内快照）；浮盈=市值−成本（未扣卖出费用），与盘后对账卡同口径；今日盈亏=老仓(昨收口径)+今日新仓(成本口径)合计，老仓行情缺昨收时不计入（宁缺勿错）；dd\_pct=虚拟权益较 live\_equity\_peak 回撤%（盘中轮询负责更新峰值，/summary 只读，创新高记 0） |
| `POST /api/live/reset`               | 清空信号机数据 `{keep_config}`（信号/回填/持仓/池子/盘中状态机快照/KV）                                                                                                                                                                                    |
| `GET /api/live/closed-trades`        | **平仓复盘（A+B）**：回填流水 FIFO 配对（口径同回测 execute\_sell，一笔卖出跨批次拆多行）→ `rows[]`（开/平日、持有交易日数、开/平价、净盈亏=扣双边费用、收益率、清仓信号类型+理由、平仓后 T+5/T+10 收盘相对平仓价%）+ `summary`（批次/累计净盈亏/胜率/平均收益率/卖对率5日/10日）+ `by_kind`（做T=当日买卖，其余按卖出信号类型；各：次数/胜率/平均收益率/累计盈亏）。日线缺失的票 hold\_days/走势退 None（宁缺勿错） |
...
- 顶层字段 `index_gate: bool = false`：中证500（000905）收盘 < MA{index_gate_ma} **连续 2 日** → 抑制开新仓/加仓（试仓升级、金字塔加仓、正向T一并抑制）；持仓退出（止损/止盈/清仓/衰退退出）与反向做T照常；
- 顶层字段 `index_gate_ma: int = 20`：大盘闸门均线周期（可填 20/30/60 等，min 2）；恢复线 = MA×1.01（内置缓冲带）；

- 恢复：收盘 ≥ MA{index_gate_ma}×(1+1%) **连续 2 日**（滞回缓冲带内置防抖）；中间地带保持现状；确认天数/缓冲带内置不开放（避免过拟合旋钮），MA 周期已开放为 `index_gate_ma`；
...
查询参数（全部可选）：`page`（默认 1）/`page_size`（默认 20，上限 1000）——服务端分页；`search`——按任务名称/任务 ID 模糊匹配（SQL LIKE）；`tag`——标签精确筛选（`tag=重点` 只看重点、`tag=__none__` 只看无标签）；`include_config`（默认 0）——=1 时 items 才含完整 config（"存为模板"需要，响应体差一个量级）。

响应：`{"items":[{"task_id","name","status(pending|running|success|failed)","created_at","tag(重点任务标签,空=无)","strategy_id","period","config(?include_config=1 时)","error"}],"total","page","page_size"}`（items 按 created_at 倒序）
